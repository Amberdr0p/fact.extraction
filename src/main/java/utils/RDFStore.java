...
public class RDFStore {
	 ...
	 * "http://winghouse.semiot.ru:3030/blazegraph/sparql"; private static final
	 * String TRIPLESTORE_USERNAME = "admin"; private static final String
	 * TRIPLESTORE_PASSWORD = "pw";
	 */
	...
	public void save(String graphUri, Model model) {
		DatasetAccessorFactory.createHTTP(ServiceConfig.CONFIG.storeUrl(), httpAuthenticator).add(graphUri, model);
	}
	...
	public ResultSet select(Query query) {
		...
		ResultSet rs = QueryExecutionFactory
				.createServiceRequest(ServiceConfig.CONFIG.storeUrl(), select, httpAuthenticator).execSelect();
		return rs;
	}
	...
	public void update(String update) {
		...
		UpdateExecutionFactory.createRemote(updateRequest, ServiceConfig.CONFIG.storeUrl(), httpAuthenticator)
				.execute();
	}
